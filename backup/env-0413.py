...
class HighwayEnv(gym.Env):
    def __init__(self,
        ...
        self.lanemarkers = np.array([])      # Lane marker positions (y-values)
        self.boundarylines = np.array([])      # Two values: [lower_boundary, upper_boundary]
        ...
        self.M1 = 7
        # Time-independent features remain: [vehicle_length, vehicle_width, agent_indicator]
        self.M2 = 3
        ...
        self.kinematics = torch.zeros(self.N_max, self.M1)
        ...
        self.features = torch.zeros(self.N_max, self.M2)
    ...
    def reset(self):
        ...
        if self.expert_data is not None:
            ...
            self.lanecenters = np.concatenate((self.boundarylines[:1], self.lanemarkers, self.boundarylines[-1:]))
            ...
            for i, (_, row) in enumerate(first_frame.iterrows()):
                ...
                self.kinematics[i] = torch.tensor([x, y, xVel, yVel, length, width, vehicle_id], dtype=torch.float32)
                self.exists[i] = True
                is_agent = bool(row['agent_status'])
                self.agent_mask[i] = is_agent
                self.features[i] = torch.tensor([length, width, 1.0 if is_agent else 0.0], dtype=torch.float32)
            ...
            self.features[0] = torch.tensor([self.default_length, self.default_width, 1.0 if is_agent else 0.0], dtype=torch.float32)
    ...
    def step(self, actions):
        ...
        updated_state = torch.cat([new_positions, new_velocities, dims, ids], dim=1)
        ...
        if self.boundarylines.size == 2:
            ...
            self.kinematics[collide, 2:4] = 0.0
        ...
        if self.expert_data is not None and self.expert_frame_idx < len(self.expert_data['frames']):
            ...
            for idx, row in expert_frame.iterrows():
                ...
                if empty_slots.numel() > 0:
                    ...
                    length = row['width']
                    width = row['height']
                    self.kinematics[slot] = torch.tensor([x, y, xVel, yVel, length, width, vehicle_id], dtype=torch.float32)
                    self.exists[slot] = True
                    is_agent = bool(row['agent_status'])
                    self.agent_mask[slot] = is_agent
                    self.features[slot] = torch.tensor([length, width, 1.0 if is_agent else 0.0], dtype=torch.float32)
